import {
    Box, Card, CardContent, LinearProgress,
...
import DMIEventsTable from "../components/DMIEventsTable";
import { DMI_ALL_COLUMNS } from "../constants/dmiColumns";
import { useAppContext } from "../context/AppContext";

const DMIEvents = forwardRef((props, ref) => {
    ...
    const [visibleKeys, setVisibleKeys] =
        useState(DMI_ALL_COLUMNS.map(c => c.key));
    ...
    const generate = async () => {
        ...
        try {
            ...
            const res = await fetch(
                `${API_BASE}/api/dmi-events/by-date`,
                {
                    method: "POST",
                    body: fileBuffer,
    ...
    useImperativeHandle(ref, () => ({
        ...
        getAllRows: () => rows,
        getVisibleColumns: () =>
            DMI_ALL_COLUMNS.filter(c => visibleKeys.includes(c.key)),
    ...
    const filtered = allRows.filter(r =>
        String(r.loco_id).includes(v) ||
        String(r.nms_system_id).includes(v) ||
        String(r.event_id).includes(v)
    );
    ...
    return (
        <Box component={motion.div} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <ColumnFilterDialog
                ...
                column="DMI Columns"
                values={DMI_ALL_COLUMNS.map(c => c.label)}
                selectedValues={visibleKeys.map(k =>
                    DMI_ALL_COLUMNS.find(c => c.key === k)?.label
                )}
                onClose={() => setColumnDialogOpen(false)}
                onApply={(labels) => {
                    const keys = DMI_ALL_COLUMNS
