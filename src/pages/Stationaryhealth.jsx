...
const StationaryHealth = forwardRef(({ healthType }, ref) => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}${endpoint}?from=${from}&to=${to}`, {
        method: "POST",
        body: fileBuffer,
        headers: {
      ...
      >
        ...
        >
          <Stack direction="row" alignItems="center" spacing={2}>
            ...
            >
              <HealthAndSafetyIcon
                sx={{ color: theme.palette.primary.light, fontSize: 26 }}
              />
            </Box>
            <Box>
              <Typography
                variant="subtitle1"
                fontWeight={500}
                sx={{ color: "#fff", lineHeight: 0.8, letterSpacing: 0.5 }}
              >
                {healthType === "STATIONARY"
                  ? "STATIONARY HEALTH"
                  : "ONBOARD HEALTH"}
