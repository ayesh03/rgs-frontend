...
import { Box, Tabs, Tab, Paper, useTheme, Tooltip } from "@mui/material";
...
export default function RSSI() {
  ...
  const handleExport = (format, all = false) => {
    ...
    const fileName = tab === 0 ? "rssi_loco" : "rssi_stationary";
  ...
  return (
    ...
    >
      <ReportHeader
        ...
        onSearch={(v) => tabRefs[tab]?.current?.searchByRSSI?.(v)}
      />
      ...
      >
        ...
        >
          <Tooltip title="View loco RSSI data">
            <Tab label="Loco RSSI" />
          </Tooltip>

          <Tooltip title="View stationary RSSI data">
            <Tab label="Stationary RSSI" />
          </Tooltip>
        </Tabs>
      </Paper>
