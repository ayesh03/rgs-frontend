...
export function formatOnboardHealth(eventId, value) {
  ...
  if (eventId === 1)
    return { name: "Radio-1 Health", desc: radioHealth[value] || value };

  if (eventId === 2)
    return { name: "Radio-2 Health", desc: radioHealth[value] || value };

  if (eventId === 3)
    return { name: "Radio-1 Input Supply", desc: formatVolt(value) };

  if (eventId === 4)
    return { name: "Radio-2 Input Supply", desc: formatVolt(value) };

  if (eventId === 9)
    return { name: "Radio-1 PA Supply Voltage", desc: formatVolt(value) };

  if (eventId === 10)
    return { name: "Radio-2 PA Supply Voltage", desc: formatVolt(value) };

  if (eventId === 5)
    return { name: "Radio-1 Temperature", desc: `${signed8(value)} °C` };

  if (eventId === 6)
    return { name: "Radio-2 Temperature", desc: `${signed8(value)} °C` };

  if (eventId === 7)
    return { name: "Radio-1 PA Temperature", desc: `${value} °C` };

  if (eventId === 8)
    return { name: "Radio-2 PA Temperature", desc: `${value} °C` };

  if (eventId === 11)
    return { name: "Radio-1 Tx PA Current", desc: formatAmp(value) };

  if (eventId === 12)
    return { name: "Radio-2 Tx PA Current", desc: formatAmp(value) };

  if (eventId === 13)
    return { name: "Radio-1 Reverse Power", desc: `${(value / 10).toFixed(1)} W` };

  if (eventId === 14)
    return { name: "Radio-2 Reverse Power", desc: `${(value / 10).toFixed(1)} W` };

  if (eventId === 15)
    return { name: "Radio-1 Forward Power", desc: `${(value / 10).toFixed(1)} W` };

  if (eventId === 16)
    return { name: "Radio-2 Forward Power", desc: `${(value / 10).toFixed(1)} W` };

  if (eventId === 17)
    return { name: "Stationary Regular Packet Received Time Offset", desc: formatMs(value) };
...
if (eventId === 43) {
  const map = {
    ...
    7: "Goods 59 BOXN Empty (75kmph)",
    8: "Goods 59 BOXN Half Load (75kmph)",
    9: "Goods 59 BOXN Full Load (60kmph)",
    10: "Goods 42 BCN Empty (75kmph)",
    11: "Goods 42 BCN Half Load (75kmph)",
    12: "Goods 42 BCN Full Load (60kmph)",
    13: "Light Engine WAP5 (170kmph)",
    14: "WAP5-8LHB Coaches (170kmph)",
    15: "Light Engine WAP7 (140kmph)"
  };

  return { name: "Selected Train Formation", desc: map[value] || value };
}
...
if (eventId === 44) {
  ...
  return { name: "Selected Cab", desc: cab[value] || value };
}
...
if (eventId === 45) {
  ...
  return { name: "Brake Application Reason", desc: reason[value] || value };
}

if (eventId === 46) {
  const stationId = value >>> 8;
  const status = value & 0xFF;
  const map = { 1: "Received", 2: "Cancelled" };

  return {
    name: "Station General SoS",
    desc: `Station ${stationId} – ${map[status] || status}`
  };
}
